...
- (instancetype)init {
    ...
    if (self) {
        self.userDefaults = [[NSUserDefaults alloc] initWithSuiteName:@"SettingsManager"];
    }
    return self;
}
...
- (UIBezierPath *)bezierPathForKey:(NSString *)key {
    ...
    if (data != nil) {
        return [NSKeyedUnarchiver unarchivedObjectOfClass:[UIBezierPath class] fromData:data error:nil];
    }
    return nil;
}
...
- (void)setBezierPath:(nullable UIBezierPath *)bezierPath forKey:(NSString *)key {
    if (bezierPath == nil) {
        ...
        NSData *data = [NSKeyedArchiver archivedDataWithRootObject:bezierPath requiringSecureCoding:NO error:nil];
